...
def splitData(numLines):
	...
	with open(features) as infile:
		...
		for i in range(numLines):
			...
			feats = row[:-1] # skip dns query name (first col)
			int_feats = []
			for feat in feats:
				int_feats.append(float(feat))
			trainingX.append(int_feats) # features
			trainingY.append(float(row[-1])) # labels
		...
		for i in range(num_rows - numLines):
			...
			testingY.append(float(row[-1])) # labels
...
def main():
	...
	(trainingX, trainingY, testingX, testingY) = splitData(130) # first 500 rows are training data, next ~200 are testing, can alter later
